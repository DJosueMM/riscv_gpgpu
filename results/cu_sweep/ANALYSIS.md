...
**Datos v3** (fix DATAFLOW aliasing activo). Los valores anteriores al fix eran
anómalos: el CU=8 original mostraba *menos* potencia que CU=2 porque HLS había
fusionado los 8 pipelines en uno solo — síntoma directo del bug.

### 6.1 Resumen total

| Métrica                  | CU=2 v3   | CU=4 v3   | CU=8 v3   |
|--------------------------|-----------|-----------|-----------|
| Total On-Chip Power (W)  | **2.718** | **2.722** | **2.925** |
| Dynamic (W)              | 2.417     | 2.422     | 2.623     |
| Device Static (W)        | 0.300     | 0.300     | 0.302     |
| Junction Temp (°C)       | 31.3      | 31.3      | 31.8      |
| Max Ambient (°C)         | 78.7      | 78.7      | 78.2      |

> El PS8 domina el consumo dinámico con ~2.215 W constante en las tres
> configuraciones. El PL (lógica programable) representa la diferencia.

### 6.2 Desglose de potencia dinámica (PL)

| Componente  | CU=2 v3 (W) | CU=4 v3 (W) | CU=8 v3 (W) |
|-------------|-------------|-------------|-------------|
| Clocks      | 0.039       | 0.040       | 0.116       |
| CLB Logic   | 0.072       | 0.073       | 0.108       |
| Signals     | 0.030       | 0.031       | 0.084       |
| Block RAM   | 0.011       | 0.013       | 0.037       |
| DSPs        | 0.000       | 0.000       | 0.014       |
| PLL         | 0.050       | 0.050       | 0.050       |
| PS8         | 2.215       | 2.215       | 2.215       |
| Static      | 0.300       | 0.300       | 0.302       |

> Con el fix, CU=8 consume ~0.4 W más que CU=2/4 en PL, proporcional a sus
> ~5.6× más LUTs y ~5× más BRAM. La tendencia es correcta: a más pipelines
> independientes activos, más potencia. El PLL (50 mW fijo) domina el overhead
> de clocking en las configuraciones pequeñas.
...
3. **Escalabilidad confirmada:** el fix de DATAFLOW muestra escalado correcto —
   ...
   La potencia PL también escala correctamente: CU=8 consume ~0.4 W más en PL
   que CU=2/4, proporcional al aumento de LUTs y BRAMs activos.
